...
std::vector<const char*> svr_addrs = {"127.0.0.1:13001"};
int batch_size = 100;
...
int main(int argc, char **argv) {
    ...
    if (number_servers > 1) {
        for (int i = 2; i <= number_servers; ++i) {
            ...
            std::string address = "127.0.0.1:620" + std::to_string(i);
    ...
    if (is_server) {
        ...
        for (int i=0; i < svr_addrs.size(); i++){
            polls[i] = new PollMgr(epoll_instances);
            // thrpools[i] = new ThreadPool(worker_threads);
            thrpools[i] = nullptr;
            BS[i]  = new SampleCrpcServiceImpl(svr_addrs[i], svr_addrs, polls[i]);
            serverVector[i] = new Server(polls[i], thrpools[i]);
            serverVector[i]->reg(BS[i]);
            verify(serverVector[i]->start(svr_addrs[i]) == 0);
        }

        for (int i=0; i < svr_addrs.size(); i++){
            // Log_info("starting the setup on the servers number: %s", svr_addrs[i]);
            BS[i]->setup(payloadSize);
        }

        BS[0]->start(outgoing_requests, seconds, fast_requests, batch_size);
